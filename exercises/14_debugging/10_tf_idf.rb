...
# The issue with the code is that we are not using float numbers for integer division in line 29. This will make the log calculation incorrect, we are getting 0 for some results of log calculation, which is incorrect. We need to use a float division instead.

# further exploration - We need to be careful if number_of_documents_with_term equals 0 as we can get Nan or infinity as result.
