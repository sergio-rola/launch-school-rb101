...
# We start by using the each method on array1 which iterates through the array1 collection and updates the array2 collection, by prepending passed objects from array1. The objects inside the collection are pointing to the same objects in array1 & array2 so when the upcase! method is called for array1 also mutates the objects of the collection in array 2.
